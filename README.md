...
AOUU 也支持一个独立的“图像区域+按键触发”。它会持续扫描用户框选的屏幕区域，并把该区域截图和用户选择的模板图片做 OpenCV 模板匹配。只有当图像相似度达到阈值，并且用户按下配置的触发按键时，才会播放对应音频。
...
1. 当前屏幕的图像区域必须匹配模板图片。
2. 用户必须按下配置的触发按键。
3. 两个条件都满足，并且不在冷却时间内，才播放触发音频。

如果图像匹配但没有按键，不会播放。如果按键按下但图像未匹配，也不会播放。状态栏会在按键时显示当前匹配相似度，方便调试阈值。
